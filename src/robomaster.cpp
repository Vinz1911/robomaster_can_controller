...
void RoboMaster::commandBlaster() {
    Message msg_infrared(DEVICE_ID_INTELLI_CONTROLLER, 0x1709, this->counter_blaster_++, { 0x00, 0x3f, 0x51, 0x01 }); // GEL Blaster
    Message msg_blaster(DEVICE_ID_INTELLI_CONTROLLER, 0x1709, this->counter_blaster_++, { 0x00, 0x3f, 0x55, 0x73, 0x00, 0xff, 0x00, 0x01, 0x28, 0x00, 0x00 }); // Infrared Blaster
    this->handler_.pushMessage(std::move(msg_infrared));
}
...
void RoboMaster::decodeDataRoboMasterState(const Message &msg) {
    if (this->callback_data_robomaster_state_) {
        ...
        data.velocity = decodeDataVelocity(27, msg);
        data.battery = decodeDataBattery(51, msg);
        data.esc = decodeDataEsc(61, msg);
        data.imu = decodeDataImu(97, msg);
        data.attitude = decodeDataAttitude(121, msg);
        data.position = decodeDataPosition(133, msg);
