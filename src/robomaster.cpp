...
namespace robomaster_can_controller {
    ...
    void RoboMaster::commandBlaster(const BlasterType blaster) {
        Message msg(DEVICE_ID_INTELLI_CONTROLLER, 0x1709, this->counter_blaster_++);
        switch (blaster) {
            case INFRARED:
                msg.setPayload({ 0x00, 0x3f, 0x55, 0x73, 0x00, 0xff, 0x00, 0x01, 0x28, 0x00, 0x00 });
                break;
            case GELBEADS:
                msg.setPayload({ 0x00, 0x3f, 0x51, 0x01 });
                break;
        }
        this->handler_.pushMessage(std::move(msg));
    }
