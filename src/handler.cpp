...
void Handler::runReceiverThread() {
    ...
    while(error_counter <= STD_MAX_ERROR_COUNT && !this->flag_stop_) {
        ...
        if(msg.length == 0) {
            ...
            // extract crc16
            const uint16_t crc16 = littleToUint16(msg.buffer[msg.length - 2], msg.buffer[msg.length - 1]);
...
void Handler::runSenderThread() {
    ...
    while (error_counter <= STD_MAX_ERROR_COUNT && !this->flag_stop_) {
        ...
        if (heartbeat_10ms_time_point < std::chrono::high_resolution_clock::now()) {
            ...
            if (msg.isValid()) {
                if (this->sendMessage(msg)) { error_counter = 0; } else { error_counter++; }
            }
        } else {
...
void Handler::runHandlerThread() {
    while (!this->flag_stop_) {
        if (!this->queue_receiver_.empty()) {
            ...
            if (msg.isValid()) { this->processReceivedMessage(msg); }
        } else {
...
void Handler::processReceivedMessage(const Message &msg) {
    if (msg.getDeviceId() == DEVICE_ID_MOTION_CONTROLLER) {
        ...
        case 0x0903:
            ...
            if(4 < msg.getPayload().size() && msg.getPayload()[0] == 0x20 && msg.getPayload()[1] == 0x48 && msg.getPayload()[2] == 0x08 && msg.getPayload()[3] == 0x00 && this->callback_data_robomaster_state_) {
